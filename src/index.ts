...
// 抽象クラスの設定
// 具体的な実装をしない場合は、interfaceを用いても良い
abstract class Game {
...
class HitAndBlow implements Game {
    ...
    private validate(inputArr: string[]) {
        ...
        const isAllDefferentValues = inputArr.every((val, i) => inputArr.indexOf(val) === i);
    ...
    async play() {
        const answerLength = this.getAnswerLength();
        const inputArr = (await promptInput(`「,」区切りで${answerLength}個の数字を入力してください`)).split(',');
...
class Janken implements Game {
    ...
    async play() {
        const userSelected = await promptSelect(`［ ${this.rounds} 回戦］選択肢を入力してください`, jankenOptions);
        const randomSelected = jankenOptions[Math.floor(Math.random() * jankenOptions.length)];
